...
identificador: primeiroIdent=IDENT (reg='.' IDENT)* dimensao;
...
declaracao_global: abreProc='procedimento' IDENT '(' (parametros)? ')' (declaracao_local)* (cmd)* fechaProc='fim_procedimento'
                    | abreFunc='funcao' IDENT '(' (parametros)? ')' ':' tipo_estendido (declaracao_local)* (cmd)* fechaFunc='fim_funcao';
...
cmdleia: 'leia' '(' ('ponteiro=^')? identificador (',' ('^')? identificador)* ')';
...
fator_logico: (neg='nao')? parcela_logica;
